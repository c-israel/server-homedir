...
require("lazy").setup({
  ...
  spec = {
    ...
    { "ravitemer/mcphub.nvim",
      cond = vim.env.NVIM_AI == "1" and vim.fn.executable("npm") == 1,
      dependencies = {
    ...
    { "NeogitOrg/neogit",
      ...
      keys = {
        ...
        { "<leader>gN", '<cmd>Neogit cwd=%:p:h<cr>', desc = 'Git: open Neogit (CWD)'},
      },
    },
    {
    ...
    },
    {
      ...
      opts = {
        ...
        dashboard = {
          enabled = true,
          sections = { -- adjusted from examples in documentation
            { section = "header", },
            { section = "keys", padding = 1,},
            { icon = " ", title = "Recent Files", section = "recent_files", pane = 2, indent = 2, padding = 1 },
            { icon = " ", title = "Projects", section = "projects", pane = 2, indent = 2, padding = 1 },
            { icon = " ", title = "Git Status", section = "terminal", pane = 2, indent = 3, padding = 1, height = 7,
                 enabled = function() return Snacks.git.get_root() ~= nil end,
                 cmd = "git --no-pager diff --stat -B -M -C",
                 ttl = 5 * 60,
               },
            { icon = " ", title = "Git Logs", section = "terminal", pane = 1, indent = 3, padding = 1, height = 5, width = 124,
                 enabled = function() return Snacks.git.get_root() ~= nil end,
                 cmd = "git --no-pager log --max-count=5 --pretty=format:\"%C(yellow)%h%C(reset) %C(auto)%d%C(reset) %s %C(cyan)(%cr)%C(reset)\"",
                 ttl = 5 * 60,
               },
            { section = "startup" },
          },
